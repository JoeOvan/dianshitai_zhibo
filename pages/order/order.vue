		...
		<view class="content" :style="{'top':tabFix}">
			...
			<swiper class="swiper-box-list" duration="300" @change="swiperChange" :current="currentIndex" :style="{'height': swiperHeight +'px'}">
				<block v-for="(item,index) in goodsData" :key="index">
					<swiper-item>
						<scroll-view class="swiper-topic-list" scroll-y="true" :style="{'height': swiperContentHeight + 'px'}" @scrolltolower="loadMore()" lower-threshold="50"  refresher-enabled="true" :refresher-triggered="triggered"
											...
											<view class="order-content-one">
												<view class="cart-goods-item" v-for="(item2,index2) in item1.orderDetailList" :key="index2">
													...
													<view class="goods-info">
														...
														<view class="intro-wrap" @click="toPage(item1.orderId,item1.orderStatus)"><text class="intro" v-if="item2.skuName!=null">{{item2.skuName}}</text></view>
														<view class="price-wrap" @click="toPage(item1.orderId,item1.orderStatus)">
		...
		methods: {
			...
			initSwiperHeight(element) {
				uni.createSelectorQuery().in(this).select(element).boundingClientRect(result => {
					...
					if (result) {
						if(result.height>0){
							this.swiperHeight = result.height+30;
						}
			...
			async toPay(orderSn) {
				...
				const res = await uni.$http.get(uni.$url.preOrderSingle, { orderSn: orderSn });
