			...
			<scroll-view scroll-y="true" class="order-detail-content" :style="{'height': contentHeihgt+'px'}">
				...
				<view class="order-detail-wrap">
					<view class="order-goods-list">
						<block v-for="(item,index) in order.orderDetailList" :key="index" :data-id="index">							
				...
				<view class="cart-wrap">
					<view class="price-wrap">共{{goodsNum}}件，合计<text>¥</text><text style="font-weight: bold;font-size: 28px;">{{order.totalMoney}}</text></view>
		...
		methods: {
			...
			async getCatrDetail() {
				...
				for(let i=0;i<this.order.orderDetailList.length;i++){
					this.goodsNum+=this.order.orderDetailList[i].num;
				}
			},
			onInputPostscript(e) {
